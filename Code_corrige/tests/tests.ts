...
test("annulation refusee a moins de 2h avec la politique standard", async () => {
  ...
  const r = await service.annuler({ reservationId: "R-009", emailMembre: "karim@exemple.fr" });
  assert.equal(r.ok, false);
...
test("annulation a exactement 2h : acceptee (cas limite)", async () => {
  ...
  const r = await service.annuler({ reservationId: "R-010", emailMembre: "karim@exemple.fr" });
  assert.equal(r.ok, true);
});
